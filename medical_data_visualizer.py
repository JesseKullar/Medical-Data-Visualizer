...
def draw_cat_plot():
    ...
    df_cat = df_cat.groupby(by=['cardio', 'variable', 'value']).size().to_frame('total').reset_index()
...
def draw_heat_map():
    ...
    ax = sns.heatmap(corr, mask=mask, annot=True, square=True, linewidth=1, vmax=0.3, fmt='.1f')
